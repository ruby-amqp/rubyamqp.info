...
Different applications handle connection failures differently. It is very common to use <span class="highlight">AMQP::Session#reconnect</span> methodto schedule a reconnection to the same host, or use {AMQP::Session#reconnect_to} to connect to a different one.
