...
A <span class="note">binding</span> is an association between a queue and an exchange. A queue must be bound to at least one exchange in order to receive messages from publishers. Learn more about bindings in the "Bindings guide":/articles/bindings/.
...
Before we start looking at various exchange types and their routing semantics, we need to introduce message attributes. Every AMQP message has a number of <span class="note">attributes</span>. Some attributes are important and used very often, others are rarely used. AMQP message attributes are metadata and are similar in purpose to HTTP request and response headers.

Every AMQP v0.9.1 message has an attribute called <span class="note">routing key</span>. The routing key is an "address" that the exchange may use to decide how to route the message . This is similar to, but more generic than, a URL in HTTP. Most exchange types use the routing key to implement routing logic, but some ignore it and use other criteria (e.g. message content).
 ...
 * By instantiating an {% yard_link AMQP::Exchange %} and specifying the type as ":fanout"
 * By using the {% yard_link AMQP::Channel#fanout %}  method
...
Both methods asynchronously declare a queue. Because the declaration necessitates a network round-trip, publishing operations on {% yard_link AMQP::Exchange %} instances are delayed until the broker reply (`exchange.declare-ok`) is received.
...
A direct exchange delivers messages to queues based on a <span class="note">message routing key</span>, an attribute that every AMQP v0.9.1 message contains.
 ...
 * By instantiating a {% yard_link AMQP::Exchange %} and specifying its type as ":direct"
 * By using the {% yard_link AMQP::Channel#direct %} method
...
Both methods asynchronously declare a queue. Because the declaration necessitates a network round trip, publishing operations on {% yard_link AMQP::Exchange %} instances are delayed until a broker reply (`exchange.declare-ok`) is received.
 ...
 * Using the {% yard_link AMQP::Channel#default_exchange %} method
 * Using the {% yard_link AMQP::Channel#direct %} method with an empty string as the exchange name

{% yard_link AMQP::Exchange#initialize %} can also be used, but requires more coding effort and it offers no benefits over instance methods on {% yard_link AMQP::Channel %} in this particular case.
...
{% example working_with_exchanges/round_robin_balancing_using_the_default_exchange.rb, 1041778 %}
...
In the example above we bind a queue with the name of "americas.south" to the topic exchange declared earlier using the {% yard_link AMQP::Queue#bind %} method. This means that only messages with a routing key matching "americas.south.#" will be routed to the "americas.south" queue.
 ...
 * By using the {% yard_link AMQP::Exchange#initialize %} method that takes an optional callback
 * By using a number of convenience methods on {% yard_link AMQP::Channel %} instances:

  ** {% yard_link AMQP::Channel#direct %}
  ** {% yard_link AMQP::Channel#default_exchange %}
  ** {% yard_link AMQP::Channel#topic %}
  ** {% yard_link AMQP::Channel#fanout %}
  ** {% yard_link AMQP::Channel#headers %}
...
To publish a message to an AMQP exchange, use {% yard_link AMQP::Exchange#publish %}:

<pre>exchange.publish("Some payload")</pre>


{% yard_link AMQP::Exchange#publish %} can accept any object that responds to the `to_s` method, not just string instances:
...
All other attributes can be added to a _headers table_ (in Ruby parlance, headers hash) that {% yard_link AMQP::Exchange#publish %} accepts as the ":headers" argument.

An example to show how message metadata attributes are passed to {% yard_link AMQP::Exchange#publish %}:
...
Sometimes it is convenient to execute an operation after publishing a message. For this, {% yard_link AMQP::Exchange#publish %} provides an optional callback. It is important to clear up some expectations of when exactly it is run and how it is related to topics of message  delivery reliability and so on.
 ...
 * {% yard_link AMQP::Exchange#publish %} takes a message and various metadata attributes
 * {% yard_link AMQP::Exchange#publish %} internally calls #to_s on the message argument to get message payload
...
Given all of this, you may ask "when does the {% yard_link AMQP::Exchange#publish %} callback fire?" The answer is on the next event loop tick. By then the data is pushed down to the OS kernel. As far as the Ruby library is concerned, it is reasonably safe behavior.

<div class="alert alert-error">
The {% yard_link AMQP::Exchange#publish %} callback is fired on the next event loop tick. Data is staged for delivery immediately. Applications MUST NOT assume that by the time the callback has fired, the data is guaranteed to leave the local machine networking stack, reach the AMQP broker or any peer applications that the message needs to be routed to.
</div>
...
An example of {% yard_link AMQP::Exchange#publish %} being used to publish an immediate message:
...
Returned messages contain information about the exchange they were published to. For convenience, the amqp gem associates returned message callbacks with {% yard_link AMQP::Exchange %} instances. To handle returned messages, use {% yard_link AMQP::Exchange#on_return %}:
...
To publish a persistent message, use the ":persistent" option that {% yard_link AMQP::Exchange#publish %} accepts:

<pre>exchange.publish(payload, :persistent => true)</pre>

<div class="alert alert-error">Note that in order to survive a broker crash, both the message and the queue that it was routed to must be persistent/durable.</div>
...
When using amqp gem in multi-threaded environments, the rule of thumb is: avoid sharing {% yard_link AMQP::Channel %} instances across threads.

Starting with 0.8.0.RC14, {% yard_link AMQP::Exchange#publish %} synchronizes data delivery on the channel object associated with exchange. This protects application developers from the most common problems related to publishing messages on a shared channel from multiple threads, however, by no means protects from every possible concurrency hazard.

<div class="alert alert-error">When using amqp gem in multi-threaded environments, the rule of thumb is: avoid sharing {% yard_link AMQP::Channel %} instances across threads.</div>

h3. Sending one-off messages

The following example publishes a message and *safely* closes the AMQP connection afterwards by passing a block to {% yard_link AMQP::Exchange#publish %} :

{% example working_with_exchanges/publishing_one_off_messages_with_ruby_amqp_gem.rb, 1020425 %}

h2. Headers exchanges

Now that message attributes and publishing have been introduced, it is time to take a look at one more core exchange type in AMQP v0.9.1. It is called {% yard_link headers %} exchange type and is quite powerful.
 ...
 * By instantiating {% yard_link AMQP::Exchange %} and specifying type as ":headers"
 * By using the {% yard_link AMQP::Channel#headers %} method
...
Both methods asynchronously declare a queue. Because declaration necessitates a network round trip, publishing operations on {% yard_link AMQP::Exchange %} instances are delayed until the broker reply (`exchange.declare-ok`) is received.
...
Queues are bound to exchanges using the {% yard_link AMQP::Queue#bind %} method. This topic is described in detail in the "Working with queues":/articles/working_with_queues/ documentation guide.


h2. Unbinding queues from exchanges

Queues are unbound from exchanges using the {% yard_link AMQP::Queue#unbind %} method. This topic is described in detail in the "Working with queues":/articles/working_with_queues/ documentation guide.

h2. Deleting exchange

h3. Explicitly deleting an exchange

Exchanges are deleted using the {% yard_link AMQP::Exchange#delete %} method:

<pre>exchange.delete</pre>

{% yard_link AMQP::Exchange#delete %} takes an optional callback that is run when a `exchange.delete-ok` reply arrives from the broker.
