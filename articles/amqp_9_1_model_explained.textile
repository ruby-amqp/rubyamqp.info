...
Each consumer (subscription) has an identifier called a _consumer tag_. This can be used to unsubscribe from messages. Consumer tags are just strings.

h2. AMQP Message Attributes and Payload

Messages in the AMQP Model have _attributes_. Some attributes are so common that the AMQP 0.9.1 specification defines them and application developers do not have to think about the exact attribute name. Some examples are
