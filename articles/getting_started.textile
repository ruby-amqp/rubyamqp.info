...
This guide is a quick tutorial that helps you to get started with the AMQP 0.9.1 specification in general and the "Ruby amqp gem":http://github.com/ruby-amqp/amqp in particular.
...
<div class="alert alert-error"><strong>Note:</strong> The RabbitMQ package that ships with recent Ubuntu versions (for example, 10.10) is outdated and *will not work with amqp gem 0.8.0 and later versions* (you will need at least RabbitMQ v2.0 for use with this guide).</div>
...
The {% yard_link AMQP::Queue#subscribe %} callback does not have to be a block. It can be any Ruby object that responds to `call`. A common technique is to combine "Object#method":http://rubydoc.info/stdlib/core/1.8.7/Object:method and "Method#to_proc":http://rubydoc.info/stdlib/core/1.8.7/Method:to_proc and use object methods as message handlers.
