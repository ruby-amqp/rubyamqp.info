...
<b>Note:</b> The RabbitMQ package that ships with recent Ubuntu versions (for example, 10.10) is outdated and *will not work with v0.8.0 and later of the amqp gem* (we need at least RabbitMQ v2.0 for use with this guide).
</span>
...
Let us begin with the classic "Hello, world" example. First, here is the code (or you can view it on <a href="/articles/amqp/hello_world/index.html">rocco-style page</a>):
...
<span class="help-block">(if the example above isn't displayed, see this "gist":https://gist.github.com/998690)</span>
...
pre. queue    = channel.queue("amqpgem.examples.helloworld", :auto_delete => true)
...
pre. exchange.publish "Hello, world!", :routing_key => queue.name
...
<span class="help-block">(if the example above isn't displayed, see this "gist":https://gist.github.com/998692)</span>
...
pre. exchange = channel.fanout("nba.scores")
...
pre. exchange.publish("BOS 101, NYK 89").publish("ORL 85, ALT 88")
...
<span class="help-block">(if the example above isn't displayed, see this "gist":https://gist.github.com/998694)</span>

The first line that is different from the Blabbr example is

pre. exchange = channel.topic("pub/sub", :auto_delete => true)
...
pre. @queue.subscribe(&@consumer.method(:handle_message))
...
pre. @consumer.method(:handle_message)
...
If you are migrating your application from earlier versions of the amqp gem (0.6.x and 0.7.x), to 0.8.x and later, there is the {file:docs/08Migration.textile amqp gem 0.8 migration guide}.
